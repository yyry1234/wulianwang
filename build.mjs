...
const raw = fs.readFileSync('/Users/lujunjian/Documents/题库练习/questions.json', 'utf-8');
const questions = JSON.parse(raw);

const css = `
*{margin:0;padding:0;box-sizing:border-box}
:root{--bg:#0a0e1a;--card:#111827;--card2:#1a2235;--border:#1e3a5f;--cyan:#00e5ff;--purple:#a855f7;--green:#22c55e;--red:#ef4444;--text:#e2e8f0;--text2:#94a3b8;--white:#fff;--glow:0 0 15px rgba(0,229,255,.3);--glow2:0 0 15px rgba(168,85,247,.3)}
...
body::before{content:'';position:fixed;top:0;left:0;width:100%;height:100%;background:radial-gradient(ellipse at 20% 50%,rgba(0,229,255,.06) 0%,transparent 50%),radial-gradient(ellipse at 80% 20%,rgba(168,85,247,.06) 0%,transparent 50%),radial-gradient(ellipse at 50% 80%,rgba(0,229,255,.04) 0%,transparent 50%);z-index:0;pointer-events:none}
.app{position:relative;z-index:1;max-width:960px;margin:0 auto;padding:20px}
.header{text-align:center;padding:40px 0 30px}
.header h1{font-size:2.2rem;background:linear-gradient(135deg,var(--cyan),var(--purple));-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-weight:800;letter-spacing:2px}
...
.result-panel .detail{color:var(--text2);margin:10px 0;font-size:.95rem;line-height:1.8}
...
::-webkit-scrollbar{width:6px}::-webkit-scrollbar-track{background:var(--bg)}::-webkit-scrollbar-thumb{background:var(--border);border-radius:3px}::-webkit-scrollbar-thumb:hover{background:var(--cyan)}
@media(max-width:640px){.header h1{font-size:1.5rem}.mode-btn{min-width:140px;padding:14px 18px}.q-card{padding:16px}}
`;

const js = `
const QUESTIONS = __JSON_PLACEHOLDER__;

let mode='review', filterKind='all', searchText='', examScope='random';
let examQuestions=[], examAnswers={}, examTimer=null, examSeconds=0;
...
function getFiltered(){
  return QUESTIONS.filter(q=>{
    ...
    if(searchText&&!q.stem.toLowerCase().includes(searchText)) return false;
    return true;
  });
}
...
function renderReview(){
  ...
  list.innerHTML=qs.map(q=>renderQCard(q,false)).join('');
}

function escapeHtml(t){return t.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}

function renderQCard(q,isExam){
  const opts=q.options.map(o=>{
    return '<div class="q-opt" data-qid="'+q.id+'" data-key="'+o.key+'" '+(isExam?'onclick="selectAnswer('+q.id+',\''+o.key+'\','+(q.kind==='multi')+')':'')+' id="opt-'+q.id+'-'+o.key+'"><span class="key">'+o.key+'.</span><span>'+escapeHtml(o.text)+'</span></div>';
  }).join('');
  ...
  const explain='<div class="q-explain" id="explain-'+q.id+'"><strong>✅ 答案：'+q.answer.join(',')+'</strong><br>'+escapeHtml(q.explanation)+'</div>';
  return '<div class="q-card" id="qcard-'+q.id+'"><div class="q-header"><span class="q-num">#'+q.id+'</span>'+kindLabel+'</div><div class="q-stem">'+escapeHtml(q.stem)+'</div><div class="q-options">'+opts+'</div>'+answerBtn+explain+'</div>';
}
...
function startExam(){
  ...
  document.getElementById('resultPanel').style.display='none';
  document.getElementById('questionList').innerHTML=examQuestions.map(q=>renderQCard(q,true)).join('');
  updateExamProgress();
...
function selectAnswer(qid,key,isMulti){
  ...
  document.querySelectorAll('[data-qid="'+qid+'"]').forEach(o=>{
    o.classList.toggle('selected',examAnswers[qid].includes(o.dataset.key));
  });
  updateExamProgress();
}
...
function submitExam(){
  ...
  document.getElementById('resultDetail').innerHTML='共 '+total+' 题，答对 <span style="color:var(--green);font-weight:700">'+correct+'</span> 题，答错 <span style="color:var(--red);font-weight:700">'+(total-correct)+'</span> 题<br>用时 '+Math.floor(examSeconds/60)+'分'+(examSeconds%60)+'秒';
  examQuestions.forEach(q=>{
    const userAns=(examAnswers[q.id]||[]).sort().join('');
    const rightAns=[...q.answer].sort().join('');
    document.querySelectorAll('[data-qid="'+q.id+'"]').forEach(o=>{
...
<div class="app">
  ...
  <div class="mode-nav">
    <div class="mode-btn active" onclick="switchMode('review')"><span class="icon">📖</span><div class="title">在线复习</div><div class="desc">逐题练习，查看答案解析</div></div>
    <div class="mode-btn" onclick="switchMode('exam')"><span class="icon">📝</span><div class="title">模拟考试</div><div class="desc">随机抽题，限时作答</div></div>
  </div>
...
const finalHtml = htmlContent.replace('__JSON_PLACEHOLDER__', raw);
fs.writeFileSync('/Users/lujunjian/Documents/题库练习/index.html', finalHtml, 'utf-8');
console.log('Written index.html, size:', finalHtml.length, 'bytes');
